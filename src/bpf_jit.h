...
static inline bool is_creg(u8 reg) {
  return (1 << reg) &
         (BIT(RV_REG_FP) | BIT(RV_REG_S1) | BIT(RV_REG_A0) | BIT(RV_REG_A1) |
          BIT(RV_REG_A2) | BIT(RV_REG_A3) | BIT(RV_REG_A4) | BIT(RV_REG_A5));
}
...
static inline int ninsns_rvoff(int ninsns) { return ninsns << 1; }
...
static inline void emitc(const u16 insn, struct rv_jit_context *ctx) {
  ...
  if (ctx->insns) ctx->insns[ctx->ninsns] = insn;
...
static inline bool is_7b_uint(unsigned long val) { return val < (1UL << 7); }

static inline bool is_8b_uint(unsigned long val) { return val < (1UL << 8); }

static inline bool is_9b_uint(unsigned long val) { return val < (1UL << 9); }

static inline bool is_10b_int(long val) {
  return -(1L << 9) <= val && val < (1L << 9);
}

static inline bool is_10b_uint(unsigned long val) { return val < (1UL << 10); }
...
static inline u32 rv_i_insn(u16 imm11_0, u8 rs1, u8 funct3, u8 rd, u8 opcode) {
  return (imm11_0 << 20) | (rs1 << 15) | (funct3 << 12) | (rd << 7) | opcode;
}
...
static inline u16 rv_cl_insn(u8 funct3, u32 imm_hi, u8 rs1, u32 imm_lo, u8 rd,
                             u8 op) {
  return (funct3 << 13) | (imm_hi << 10) | ((rs1 & 0x7) << 7) | (imm_lo << 5) |
         ((rd & 0x7) << 2) | op;
}
...
static inline u16 rvc_addi4spn(u8 rd, u32 imm10) {
  ...
  imm = ((imm10 & 0x30) << 2) | ((imm10 & 0x3c0) >> 4) | ((imm10 & 0x4) >> 1) |
        ((imm10 & 0x8) >> 3);
...
static inline u16 rvc_mv(u8 rd, u8 rs) { return rv_cr_insn(0x8, rd, rs, 0x2); }

static inline u16 rvc_jalr(u8 rs1) {
  return rv_cr_insn(0x9, rs1, RV_REG_ZERO, 0x2);
}

static inline u16 rvc_add(u8 rd, u8 rs) { return rv_cr_insn(0x9, rd, rs, 0x2); }
