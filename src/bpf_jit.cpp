...
static void emit_alu_i64(const s8 *dst, s32 imm, struct rv_jit_context *ctx,
  ...
  case BPF_AND:
    ...
    if (imm >= 0) emit(rv_addi(hi(rd), RV_REG_ZERO, 0), ctx);
    break;
  case BPF_OR:
    if (is_12b_int(imm)) {
    ...
    if (imm < 0) emit(rv_ori(hi(rd), RV_REG_ZERO, -1), ctx);
    break;
  case BPF_XOR:
    if (is_12b_int(imm)) {
    ...
    if (imm < 0) emit(rv_xori(hi(rd), hi(rd), -1), ctx);
    break;
  case BPF_LSH:
    if (imm >= 32) {
...
static int emit_branch_r32(const s8 *src1, const s8 *src2, s32 rvoff,
  ...
  if (emit_bcc(op, lo(rs1), lo(rs2), rvoff, ctx)) return -1;
...
static int emit_bpf_tail_call(int insn, struct rv_jit_context *ctx) {
  ...
  tc_ninsn = insn ? ctx->offset[insn] - ctx->offset[insn - 1] : ctx->offset[0];
...
static int emit_store_r64(const s8 *dst, const s8 *src, s16 off,
  ...
  if (mode == BPF_ATOMIC && size != BPF_W) return -1;
...
int bpf_jit_emit_insn(const struct bpf_insn *insn, struct rv_jit_context *ctx,
                      bool extra_pass) {
  bool is64 =
      BPF_CLASS(insn->code) == BPF_ALU64 || BPF_CLASS(insn->code) == BPF_JMP;
  ...
  case BPF_ALU | BPF_END | BPF_FROM_LE: {
    ...
    case 16:
      ...
      fallthrough;
    case 32:
      // NOTE: this line equals true, from kernel/bpf/verifier.c :
      // bpf_jit_needs_zext if (!ctx->prog->aux->verifier_zext)
      emit(rv_addi(hi(rd), RV_REG_ZERO, 0), ctx);
      break;
    case 64:
      /* Do nothing. */
  ...
  case BPF_JMP | BPF_TAIL_CALL:
    if (emit_bpf_tail_call(i, ctx)) return -1;
    break;
  ...
  case BPF_LDX | BPF_MEM | BPF_DW:
    if (emit_load_r64(dst, src, off, ctx, BPF_SIZE(code))) return -1;
    break;
  ...
  case BPF_STX | BPF_MEM | BPF_DW:
    ...
    if (emit_store_r64(dst, src, off, ctx, BPF_SIZE(code), BPF_MODE(code)))
      return -1;
    break;

  case BPF_STX | BPF_ATOMIC | BPF_W:
    if (insn->imm != BPF_ADD) {
      pr_info_once("bpf-jit: not supported: atomic operation %02x ***\n",
                   insn->imm);
      return -EFAULT;
    }
